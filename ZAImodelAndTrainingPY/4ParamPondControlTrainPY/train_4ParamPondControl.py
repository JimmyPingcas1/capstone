...
def build_schema(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["pH_Status"] = np.where(
        (df["pH"] >= 6.5) & (df["pH"] <= 8.5),
        "High",
        "Low"
    )

    df["Ammonia_Status"] = np.where(
        df["Ammonia"] <= 0.02,
        "High",
        "Low"
    )
    ...
    df["Temp_Status"] = np.where(
        (df["Temp"] >= 26) & (df["Temp"] <= 30),
        "High",
        "Low"
    )

    df["Turbidity_Status"] = np.where(
        (df["Turbidity"] >= 25) & (df["Turbidity"] <= 40),
        "High",
        "Low"
    )
    ...
    df["Water Pump"] = np.where(
        (df["Ammonia_Status"] == "Low")
        | (df["Turbidity_Status"] == "Low")
        | (df["Temp"] > 30),
        "ON",
        "OFF"
    )
...
def encode_features_targets(df_schema: pd.DataFrame):
...
def apply_label_noise(y: pd.DataFrame, noise_rate: float, seed: int):
...
def evaluate(y_true: pd.DataFrame, y_pred: np.ndarray):
...
def main():
    ...
    y_noisy, noise_counts = apply_label_noise(
        y,
